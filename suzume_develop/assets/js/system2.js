    ...
    function load(){
        ...
        const retrievedJsonText = localStorage.getItem("settingDate2");
    ...
    function setStrage(){
        ...
        localStorage.setItem("settingDate2", jsonText);
        makeSuggestPrizes();
    }
...
const reliefSaved=localStorage.getItem("relief2") === "true";

relief.checked=reliefSaved;

    relief.addEventListener('change',()=>{
        localStorage.setItem("relief2", relief.checked);
    })
...
function saveResult(result){
    let logId = 0;
    // ローカルストレージに保存
    const retrievedJsonText = localStorage.getItem("gamelog2");
    ...
    localStorage.setItem("gamelog2", jsonText);
